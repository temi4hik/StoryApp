...
public class Story {
    ...
    private Page[] mPages;
    private static final int PAGES_NUMBER = 7;

    private Story() {
        mPages = new Page[PAGES_NUMBER];
    ...
    public static synchronized Story getStory() {
        if (story == null)
            story = new Story();
    ...
    public Page getPage(int pageNumber) throws IllegalArgumentException{
        if (pageNumber >= PAGES_NUMBER)
